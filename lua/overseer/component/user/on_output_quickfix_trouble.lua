...
local function copen(self, height)
  ...
  -- 判断没有quikfix
  if #qflist == 1 then
    return vim.cmd("Trouble qflist close")
  end
  -- 排除成功执行
  local target_string = "Process exited 0"
  local contains_target = false
  for _, item in ipairs(qflist) do
    if item.text and item.text:find(target_string) then
  ...
  -- 改用trouble
  local open_cmd = "Trouble qflist open"
  if height then
